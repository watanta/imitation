...
class LuxDataset_worker(Dataset):
    ...
    def __getitem__(self, idx):
        obs_id, unit_id, action, rotate = self.samples[idx]
        obs = self.obses[obs_id]
        state = make_input_worker(obs, unit_id)
        if rotate is not None:
            state, action = rotate_state_and_action(state, action, rotate)
...
def add_rotate_marker(samples):
    rotate_makers  = [0, 90, 180, 270]
    new_samples = []
    for sample in samples:
        ...
        if action not in [0,1,2,3]:
            ...
            for marker in rotate_makers:
                new_samples.append(sample + tuple([marker]))
    return new_samples
...
if __name__ == "__main__":
    ...
    woker_samples = add_rotate_marker(woker_samples)
    citytile_samples = add_rotate_marker(citytile_samples)

    woker_labels = [sample[2] for sample in woker_samples]
    ...
    citytile_labels = [sample[2] for sample in citytile_samples]
